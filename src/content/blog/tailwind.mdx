...
<a id="ds-conception"></a> Foundations are the core base variables that shape every color, size, and
style in the product. 🎨 Once defined, they bring the first important factor: Consistency.
...
- There are a lot of `<div>` - and we _don't know at first what is doing what_, you need to read class by class to understand it, as elements are not identified (see [here](https://github.com/arthu-pr/thoughts/pull/1#naming-things))
...
- Maintainability is a concern, as we rely on an external tool (see [Tailwind v4](https://github.com/arthu-pr/thoughts/pull/1#tailwind-v4))
